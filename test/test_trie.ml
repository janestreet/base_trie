   ...
   struct
     ...
     let sexp_of_t sexp_of_chain sexp_of_data sexp_of_desc trie =
       require_does_not_raise [%here] (fun () ->
         Trie.invariant ignore ignore trie);
     ...
     let%expect_test "of_alist_or_error" =
       let test alist =
         ...
         print_s
           [%sexp
             { alist : (keychain * data) list; trie : data T.t Or_error.t }]
       in
       List.iter alists_with_duplicates ~f:test;
       [%expect
         {|
     ...
     let%expect_test "of_alist_exn" =
       let test alist =
         let trie =
           Or_error.try_with (fun () -> of_alist_exn keychainable alist)
         in
         print_s
           [%sexp
             { alist : (keychain * data) list; trie : data T.t Or_error.t }]
     ...
     let%expect_test "of_alist_multi" =
       let test alist =
         ...
         print_s
           [%sexp { alist : (keychain * data) list; trie : data list T.t }]
       in
       List.iter alists_with_duplicates ~f:test;
       [%expect
         {|
     ...
     let%expect_test "of_sequence_exn" =
       let test alist =
         let sequence = Sequence.of_list alist in
         let trie =
           Or_error.try_with (fun () -> of_sequence_exn keychainable sequence)
         in
         print_s
           [%sexp
             { sequence : (keychain * data) Sequence.t
     ...
     let%expect_test "of_sequence_multi" =
       let test alist =
         ...
         print_s
           [%sexp
             { sequence : (keychain * data) Sequence.t; trie : data list T.t }]
       in
       List.iter alists_with_duplicates ~f:test;
       [%expect
         {|
     ...
     let%expect_test "equal" =
       let sexp_of_pair (fst, snd) =
         [%sexp { fst : data T.t; snd : data T.t }]
       in
       let test tries =
         let equal, different =
           List.partition_tf
             (List.cartesian_product tries tries)
             ~f:(fun (trie1, trie2) -> equal equal_data trie1 trie2)
         in
     ...
     let%expect_test "set" =
       ...
       in
       List.iter example_key_lists ~f:(fun keychain ->
         test ~keychain ~data:"salutations");
       [%expect
         {|
         ((keychain ())
          (data salutations)
     ...
     let%expect_test "add_or_error" =
       ...
       let test ~keychain ~data =
         ...
         print_s
           [%sexp
             { keychain : keychain; data : data; trie : data T.t Or_error.t }]
       in
       List.iter example_key_lists ~f:(fun keychain ->
         test ~keychain ~data:"salutations");
       [%expect
     ...
     let%expect_test "add_exn" =
       ...
       let test ~keychain ~data =
         let trie =
           Or_error.try_with (fun () -> add_exn trie ~keychain ~data)
         in
         print_s
           [%sexp
             { keychain : keychain; data : data; trie : data T.t Or_error.t }]
     ...
     let%expect_test "set_trie" =
       ...
       let test keychain =
         let trie =
           set_trie large_example_trie ~keychain ~trie:small_example_trie
         in
         print_s [%sexp { keychain : keychain; trie : data T.t }]
       in
       List.iter example_key_lists ~f:test;
     ...
     let%expect_test "add_trie" =
       ...
       let test keychain =
         let trie =
           add_trie large_example_trie ~keychain ~trie:small_example_trie
         in
         print_s
           [%sexp
             { keychain : keychain; trie : (data T.t, keychain) Or_duplicate.t }]
       in
       List.iter example_key_lists ~f:test;
       [%expect
         {|
     ...
     let%expect_test "to_sequence" =
       let test trie =
         ...
         print_s
           [%sexp { trie : data T.t; sequence : (keychain * data) Sequence.t }]
       in
       List.iter example_tries ~f:test;
       [%expect
         {|
     ...
     let%expect_test "iter_keychains" =
       let test i trie =
         if i > 0 then print_endline "--";
         print_s [%sexp { trie : data T.t }];
         iter_keychains trie ~f:(fun keychain ->
           print_s [%sexp (keychain : keychain)])
       in
       List.iteri example_tries ~f:test;
       [%expect
         {|
     ...
     let%expect_test "fold" =
       let test trie =
         let string =
           fold trie ~init:"..." ~f:(fun acc data -> data ^ "; " ^ acc)
         in
     ...
     let%expect_test "filter" =
       let test trie =
         let filtered =
           filter trie ~f:(fun string -> String.length string % 2 = 0)
         in
     ...
     let%expect_test "filter_keychains" =
       let test trie =
         let filtered =
           filter_keychains trie ~f:(fun keychain ->
             List.length keychain % 2 = 0)
         in
         print_s [%sexp { trie : data T.t; filtered : data T.t }]
       in
       List.iter example_tries ~f:test;
     ...
     let%expect_test "for_all" =
       let test trie =
         let success =
           for_all trie ~f:(fun data -> String.is_prefix data ~prefix:"h")
         in
     ...
     let%expect_test "exists" =
       let test trie =
         let success =
           exists trie ~f:(fun data -> String.is_prefix data ~prefix:"h")
         in
         print_s [%sexp { success : bool; trie : data T.t }]
       in
       List.iter example_tries ~f:test;
     ...
     let%expect_test "count" =
       let test trie =
         let count =
           count trie ~f:(fun data -> String.is_prefix data ~prefix:"h")
         in
     ...
     let%expect_test "partition_tf" =
       let test trie =
         ...
         in
         print_s
           [%sexp { trie : data T.t; success : data T.t; failure : data T.t }]
       in
       List.iter example_tries ~f:test;
       [%expect
         {|
     ...
     let%expect_test "merge" =
       let test_pair (left, right) =
         let merged =
           merge left right ~f:(fun ~keychain variant ->
             Some (keychain, variant))
         in
         print_s
           [%sexp
             { left : data T.t
       ...
       in
       let test tries =
         List.iter (List.cartesian_product tries tries) ~f:test_pair
       in
       test example_tries;
       [%expect
         {|
     ...
     let%expect_test "merge_skewed" =
       let test_pair (left, right) =
         ...
         in
         print_s
           [%sexp { left : data T.t; right : data T.t; merged : data T.t }]
       in
       let test tries =
         List.iter (List.cartesian_product tries tries) ~f:test_pair
       in
